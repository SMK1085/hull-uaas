...
export const normalizeFacebookUrl = (raw: any): string | undefined => {
  ...
  try {
    ...
    const facebookUrl = new URL(result);
    if (facebookUrl.hostname !== "facebook.com") {
      throw new Error(
        `Hostname '${facebookUrl.hostname}' is not the expected hostname 'facebook.com'.`,
      );
    }
